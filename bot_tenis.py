...
def analizar_partidos():
    ...
    for evento in data:
        ...
        for mercado in evento.get("bookmakers", []):
            casa = mercado["title"]
            if casa not in ["FanDuel", "DraftKings", "BetMGM"]:  # Solo casas legales en Indiana
                continue

            for outcome in mercado["markets"]:
                tipo_mercado = outcome["key"]  # h2h, totals, etc.
                outcomes = outcome["outcomes"]
                for o in outcomes:
                    nombre = o["name"]
                    cuota = o["price"]

                    # Buscar mejor y peor cuota para este resultado entre todas las casas
                    mejor = cuota
                    peor = cuota
                    casa_mejor = casa
                    casa_peor = casa
